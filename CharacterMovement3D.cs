...
public class CharacterMovement3D : CharacterMovement
{
    ...
    {
        if (!CanRotate || direction.magnitude < 0.1f) return;
    ...
    {
        if (!CanJump || !CanMove || !IsFudgeGrounded) return;
    ...
    public void MoveTo(Transform destination) { StartCoroutine(MoveTrans(destination)); MoveTo(destination.position); }
    private IEnumerator MoveTrans(Transform destination) 
    { 
        float distance = Vector3.Distance(destination.position, gameObject.transform.position);
        while (distance > .5f)
        {
            Debug.DrawLine(destination.position, gameObject.transform.position, Color.cyan, 2f);
            yield return null;
    ...
    private void FixedUpdate()
    {
        ...
        if (_navMeshAgent.hasPath && _navMeshAgent.path.corners.Length > 1)
        {
            ...
            input = pathDir;
            LookDirection = pathDir;
            Debug.DrawLine(transform.position, _navMeshAgent.destination, Color.magenta);
        }
        ...
        if (_controlRotation && (IsGrounded || _airTurning) && CanRotate)
        {
            Quaternion targetRotation = Quaternion.LookRotation(LookDirection);
            Quaternion rotation = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * _turnSpeed);
        ...
        //    float dot = Mathf.Clamp01(Vector3.Dot(transform.forward, LookDirection));
        //    dot = Mathf.Pow(dot, _turnEasing);
        //    float turnMagnitude = 1f - dot;
    ...
    public void SetTurnSpeed(float _newSpeed)
    {
        _turnSpeed = _newSpeed;
    }
}